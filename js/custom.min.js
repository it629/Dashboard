...
//         // 🛠️ Make dark the default if no cookie
//         var version = getCookie('version') || 'dark';  // <--- Change here
...
var handleThemeMode = function() {
    ...
    // Ensure the toggle button looks active (if it exists)
    if (jQuery(".dlab-theme-mode").length > 0) {
        jQuery('.dlab-theme-mode').addClass('active');

        // Remove the click event to prevent toggling
        jQuery('.dlab-theme-mode').off('click');
    }

    // If there's a version dropdown, set and disable it
    if (jQuery('#theme_version').length > 0) {
        jQuery('#theme_version')
            .val('dark')
            .prop('disabled', true);
    }

    // Refresh styled dropdowns (e.g., bootstrap-select)
	...
	// 	var support = '<script id="DZScript" src="https://dzassets.s3.amazonaws.com/w3-global-2.0.js?token=W-2260f8ab7b35d8041165d50ad9a27243"></script>';
